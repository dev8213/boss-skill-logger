...
module.exports = function Bosslogger(mod) {
    ...
    mod.hook('S_ACTION_STAGE', 9, (event) => {
       // if (!mod.settings.logboss || !mod.settings.whitelist.includes(event.templateId) || event.stage > 0) return;
        ...
        if (mod.settings.writelog) {
            if (mod.settings.whitelist.includes(event.templateId)) {
                //stream.write(gettime()+' |S_ACTION_STAGE| >> '+'source: '+getId(event.gameId)+' skill: '+event.skill.id+' hp: '+bosshp+ ' w: '+round(event.w)+'\n');
	...
	mod.hook('S_ACTION_END', 5, (event) => {
        if (!mod.settings.logboss || !mod.settings.whitelist.includes(event.templateId)) return;
        //sendchat('Action End: '+`${event.skill}`.clr('ff6900')+' Skill ID: '+`${event.skill.id}`.clr('ff8000')+'.');
        if (mod.settings.writelog) {
            if (mod.settings.whitelist.includes(event.templateId)) {
